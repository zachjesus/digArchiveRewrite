...
async function loadUniversityConfig() {
    try {
        const nodeConfig = await fetch('settings/archiveNode.json').then(r => r.json());
        return nodeConfig;
    } catch (error) {
            ...
            if (itemsLoaded) {
                ...
                items.forEach((item, i) => {
                    ...
                    itemNode.bindPopup(`
                        ...
                        <img src="/arweave/data/${item.metadata.dataTxId}" style="max-width:200px;display:block;margin-bottom:8px;">
...
function linkNodes(nodeA, nodeB) {
    ...
    const rA = (nodeA.options.radius || 10) / 111000; // Convert radius to degrees
    const rB = (nodeB.options.radius || 10) / 111000; // Convert radius to degrees
    ...
    if (dist === 0) return; // Prevent division by zero if nodes overlap

    // Apply offsets to connect to the edges of the nodes
...
async function getItemsInParentFolder(driveID, parentFolderID) {
    try {
        const response = await fetch(`/arweave/folder/${driveID}?parentFolderId=${parentFolderID}`);
        if (!response.ok) {
